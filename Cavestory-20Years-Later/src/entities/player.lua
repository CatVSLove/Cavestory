...
local width, height = 4 * scale, 16 * scale

player = world:newBSGRectangleCollider(30*scale, 200*scale - height / 2, width, height, 1.5 * scale)
...
function player:OnGround()
  local querys = world:queryRectangleArea(self:getX() - self.width / 2 + 1, self:getY() + self.height / 2 - 0.25, self.width - 2, 0.5 * scale, {"platform", "enemy"})
...
function player:HitCeiling()
  local querys = world:queryRectangleArea(self:getX() - self.width / 2 + 1, self:getY() - self.height / 2 + 0.25 - 4, self.width - 2, 0.5 * scale, {"platform", "enemy"})
...
function player:update(dt)
  ...
  if not self:OnGround() then
    self.jumpFrame = self.jumpFrame or 4
...
function player:draw()
  self.currentAnimation:draw(self.spritesheet, self:getX() - 16 * (scale / 2), self:getY() - self.height / 2, nil, scale)
end
